...
@Component({
  selector: 'app-produccion-lactea-form',
  imports: [CommonModule, ReactiveFormsModule, RouterLink],
  templateUrl: './produccion-lactea-form.html',
  styleUrl: './produccion-lactea-form.scss',
})
export class ProduccionLacteaForm implements OnInit {
  cargando = false;
  cargandoDatos = false;
  error = '';
  ...
  crearBatch() {
    return this.fb.group({
      numeroBatch: [this.batches.length + 1, Validators.required],
      idMarmita: [null, Validators.required],
      litrosConsumidos: [0, [Validators.required, Validators.min(0.001)]],
      kilosProducidos: [0, [Validators.required, Validators.min(0.001)]],
      observaciones: ['']
    });
  }
  ...
  renumerarBatches(): void {
    this.batches.controls.forEach((control, index) => {
      control.get('numeroBatch')?.setValue(index + 1);
    });
  }
  ...
  cargarTanques(): void {
    ...
    this.recepcionLecheService.listarSaldosTanques().subscribe({
      next: (data) => {
        this.tanques = data.filter(tanque => tanque.activo);
        this.cargandoDatos = false;
      },
      error: (err) => {
        console.error(err);
        this.error = 'No se pudieron cargar los tanques de leche.';
        this.cargandoDatos = false;
      }
    });
  }
  ...
  guardar(): void {
    ...
    const request = {
      ...
      batches: value.batches.map((batch: any) => ({
        numeroBatch: Number(batch.numeroBatch),
        idMarmita: Number(batch.idMarmita),
        litrosConsumidos: Number(batch.litrosConsumidos),
        kilosProducidos: Number(batch.kilosProducidos),
        observaciones: batch.observaciones || undefined
      }))
    };

    this.produccionService.registrar(request).subscribe({
      next: () => {
        this.cargando = false;
        this.router.navigate(['/produccion-lactea']);
      },
      error: (err) => {
        console.error(err);
        this.cargando = false;
        this.error = 'No se pudo registrar la producción láctea.';
      }
    });
  }
}