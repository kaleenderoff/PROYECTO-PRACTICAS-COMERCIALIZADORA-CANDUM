...
public class OrdenProduccionRestMapper {
        ...
        public static OrdenProduccionResponse toResponse(OrdenProduccion orden) {
                ...
                java.math.BigDecimal kgPTTotalPlan = skus.stream()
                                .map(s -> s.getKgProductoTerminado() != null ? s.getKgProductoTerminado()
                                                : java.math.BigDecimal.ZERO)
                                .reduce(java.math.BigDecimal.ZERO, java.math.BigDecimal::add);

                java.math.BigDecimal kgEntradaTotalPlan = skus.stream()
                                .map(s -> s.getKgBatchCalculado() != null ? s.getKgBatchCalculado()
                                                : java.math.BigDecimal.ZERO)
                                .reduce(java.math.BigDecimal.ZERO, java.math.BigDecimal::add);
                ...
                response.setTandasCerradas(Boolean.TRUE.equals(orden.getTandasCerradas()));
                response.setFechaCierreTandas(orden.getFechaCierreTandas());
                response.setIdUsuarioCierreTandas(orden.getIdUsuarioCierreTandas());
                response.setNombreUsuarioCierreTandas(orden.getNombreUsuarioCierreTandas());
                response.setObservacionesCierreTandas(orden.getObservacionesCierreTandas());
