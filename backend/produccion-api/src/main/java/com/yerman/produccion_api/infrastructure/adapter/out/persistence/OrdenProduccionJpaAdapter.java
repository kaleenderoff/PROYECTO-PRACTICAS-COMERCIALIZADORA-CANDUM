...
public class OrdenProduccionJpaAdapter implements OrdenProduccionRepositoryPort {
    ...
    public OrdenProduccion guardar(OrdenProduccion domain) {
        ...
        if (domain.getId() != null) {
            entity = repository.findById(domain.getId())
                    .orElse(new OrdenProduccionEntity());
        } else {
    ...
    private void mapToEntity(OrdenProduccion domain, OrdenProduccionEntity entity) {
        ...
        entity.setProgramacion(
                entityManager.getReference(ProgramacionProduccionEntity.class, domain.getIdProgramacion()));

        entity.setLinea(
                entityManager.getReference(CatalogoLineaEntity.class, domain.getIdLinea()));

        entity.setProducto(
                entityManager.getReference(CatalogoProductoEntity.class, domain.getIdProducto()));

        entity.setTurno(
                entityManager.getReference(TurnoEntity.class, domain.getIdTurno()));

        if (domain.getIdJefeLineaEjecutor() != null) {
            entity.setJefeLineaEjecutor(
                    entityManager.getReference(UsuarioEntity.class, domain.getIdJefeLineaEjecutor()));
        } else {
            entity.setJefeLineaEjecutor(null);
        }

        entity.setCreadaPor(
                entityManager.getReference(UsuarioEntity.class, domain.getIdCreadaPor()));
        ...
        if (domain.getIdTanqueLeche() != null) {
            entity.setTanqueLeche(
                    entityManager.getReference(TanqueLecheEntity.class, domain.getIdTanqueLeche()));
        } else {
        ...
        entity.setTandasCerradas(Boolean.TRUE.equals(domain.getTandasCerradas()));
        entity.setFechaCierreTandas(domain.getFechaCierreTandas());
        entity.setObservacionesCierreTandas(domain.getObservacionesCierreTandas());

        if (domain.getIdUsuarioCierreTandas() != null) {
            entity.setUsuarioCierreTandas(
                    entityManager.getReference(UsuarioEntity.class, domain.getIdUsuarioCierreTandas()));
        } else {
            entity.setUsuarioCierreTandas(null);
        }
    }
}